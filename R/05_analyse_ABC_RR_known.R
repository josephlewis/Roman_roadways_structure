...
ggplot2::ggsave(plot_b_df, filename = "./Output/figures/road_sims_posterior_b.png", dpi = 300, width = 12, height = 8)
...
plot_cf_dfs1 <- ggplot() + 
  ...
  labs(x = "Mathematical slope gradient (rise/run)", caption = "Median posterior estimate for rate of decline from maximum influence (parameter b) shown with dashed line", y = "Influence of slope gradient", fill = "Credible Interval") +
    theme_clean() + 
...
plot_cf_dfs3 <- ggplot() + 
  ...
  scale_y_continuous(breaks = seq(0, 1, 0.25), limits = c(0, 1)) +
  scale_x_continuous(breaks = c(seq(-0.8, -0.4, 0.4), 0, seq(0.4, 0.8, 0.4))) +
  labs(x = "Mathematical slope gradient (rise/run)", caption = "Median posterior estimate for rate of decline from maximum influence (parameter b) shown with dashed red line", y = "Influence of slope gradient", fill = "Credible Interval") +
  theme_clean() + 
  theme(legend.position = "bottom", legend.justification = "right")
